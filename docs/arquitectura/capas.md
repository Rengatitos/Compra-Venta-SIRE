...
- **`services`** contiene la lógica de negocio: `propuesta_service.py` (orquesta la sincronización con el SIRE), `comprobante_service.py` (serialización hacia la API y armado del texto para la IA), `jobs_service.py` (ejecución de trabajos asíncronos con seguimiento de progreso), `detalle_service.py` (extracción de detalle vía scraping, como job), `analisis_ia.py` (integración con Gemini y RAG), `analytics_service.py` (agregaciones para el dashboard), `export_service.py` (generación de Excel/PDF), `scraping_sunat.py` (automatización del portal SOL con Playwright) y el paquete `sunat/` (cliente HTTP hacia la API oficial: `auth.py` para OAuth, `propuesta.py` para la descarga y el mapeo de la propuesta).
